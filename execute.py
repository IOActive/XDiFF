...
class Execute(object):
	...
	def run_subprocess(self, piece, testcase):
		...
		if "execute" in piece:
			try:
				...
				stdout, stderr = self.analyzeResults(stdout, stderr)
			except OSError:
	...
	def run_selenium(self, piece, input):
		"""Obtain the stdout and stderr when executing a piece of software using selenium"""
		url = testcase['execute'].pop()
		self.settings['logger'].debug("Input received: " + url)
		stdout = stderr = elapsed = returncode = ""
		start_test = time.time()
		try:
			piece['driver'].get(url)
		except Exception as e:
			self.settings['logger'].warning("Exception:" + str(e))
		elapsed = str(round(time.time() - start_test, 4))
		self.results = {"softwareid":piece['softwareid'], "testcaseid":testcase['testcaseid'], "stdout":stdout, "stderr":stderr, "returncode":returncode, "elapsed":elapsed}
