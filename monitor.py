...
class Monitor(object):
	...
	def __init__(self, settings=None):
		...
		self.canaryexec = "canaryfile.bat"
		self.canaryhost = "canaryhost"

	def check_once(self):
		"""Check only once"""
		status = self.check_canary_references(self.canaryfile)
		status += self.check_canary_references(self.canaryhost)
		status += self.check_canary_web(self.canaryhost, self.canaryfile, "canarytokenfileremote")
		status += self.check_canary_command(self.canaryexec, "canarytokencommand")
	...
	def check(self):
		...
		self.remove_stuff()
		status  = self.check_canary_file(self.settings['tmp_dir'] + self.canaryfile, "canarytokenfilelocal")
